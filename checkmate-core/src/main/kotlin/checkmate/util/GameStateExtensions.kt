        ...
        for (file in board[rank].indices) {
            ...
            if (piece != null) {
                ...
                when (piece.type) {
                    ...
                    Type.KNIGHT -> if (piece.color == Player.WHITE) bitmapGameState.whiteKnights = bitmapGameState.whiteKnights or bitPosition else bitmapGameState.blackKnights = bitmapGameState.blackKnights
...
private fun CastlingRights.calculateCastlingRights(): Int {
    ...
    if (whiteKingSide) castlingRights = castlingRights or 0b0001
    if (whiteQueenSide) castlingRights = castlingRights or 0b0010
    if (blackKingSide) castlingRights = castlingRights or 0b0100
    if (blackQueenSide) castlingRights = castlingRights or 0b1000
